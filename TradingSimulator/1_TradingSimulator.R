...
df.Positions<-f.derivePositions(df.AllTrades);
...
f.derivePositions<-function(df.AllTrades,l.prices){
   ...
   # df.SymTrades.xts<-f.df2xts(df.SymTrades)
    ...
    while(iticker<length(v.tickers)){
        ...
        N<-nrow(df.P.xts)
        df.theseTrades<-dplyr::filter(df.AllTrades,Symbol==symb);
        df.T<-df.theseTrades%>%dplyr::select(Type,Transaction,Quantity,price,Amount,Date)
        names(df.T)[1]<-"TradeType";
        
        
        df.positions<-f.initPositions(symb,N);
        df.positions$Date<-index(df.P.xts);
        df.positions$Last<-df.P.xts$Close;
        df.Pos<-merge.data.frame(df.positions,df.T,by="Date",all.x=TRUE)
      
        fnout<-"df.Pos.csv";
        write.csv(df.Pos,file=fnout)
        #loop through every day, updating the daiy positions for this stock
        t<-0;
        curquantity<-0;
         while(t<N){
            t<-t+1;
            dat<-df.Pos$Date[t];
            df.Pos$symbol[t]<-symb;
            if(!is.na(df.Pos$TradeType[t])){
                quantity<-df.Pos$Quantity[t];   #quantity of shares Traded
                tran<-df.Pos$Transaction[t];    #buy or sell
                direction<-ifelse(tran=="Buy",1,-1)
                
                ttype<-df.Pos$TradeType[t];     #Margin or Short
                oldttype<-ifelse{ttype=="Margin",1,-1);  #1 - Margin Pos, #2=Short Position}

                oldq<-df.Pos$quantity[t];
                newq<-oldq+quantity
                if(df.Pos$Transaction=="Buy"){
