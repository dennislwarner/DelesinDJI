#  Use Once utility to produce the simple price file used in THWPF from the simulated DJIA
#
...
NL<-length(l.Prices);
iticker<-0;
nrows<-nrow(l.Prices[[1]])
df.Closes<-data.frame(matrix(0,nrows,NL));

df.Closes.xts<-as.xts(df.Closes,order.by = index(l.Prices[[1]]))

while(iticker<NL){
    iticker<-iticker+1;
    df.p<- l.Prices[[iticker]];
    if(iticker==1){
        df.Closes.xts<-df.p$Close;
    }else{
        df.Closes.xts<-merge.xts(df.Closes.xts,df.p$Close,join="left")
    }
  
}
fnout<-"D:/Projects/TWPFData.csv";
names(df.Closes.xts)<-names(l.Prices);
df.Closes<-f.xts2df(df.Closes.xts);
df.Closes[is.na(df.Closes)]<-0.0;
write.csv(df.Closes,file=fnout);
fnout2<-"D:/Projects/THWPFDATA/AllPricesSimulated.csv";
write.csv(df.Closes,file=fnout2);
