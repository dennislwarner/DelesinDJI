...
f.executeTrades<-function(df.xts,symb,cname,CASH,v.signals){
    ...
    df.trades<-f.initTradeRecord(symb,cname);
    df.tranches<-f.initTranches(symb,ondate,onprice,shares);
    df.realizations<-f.initRealizations(symb)
    
    l.Z<-list(l.trades=l.trades)
